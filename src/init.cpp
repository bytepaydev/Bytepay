...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bytepay.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bytepayd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebytepayamount=<n> " + _("Keep N Bytepay anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bytepay is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bytepay is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bytepay version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bytepay server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bytepay") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bytepay to complete") << "\n";
    ...
    nAnonymizeBytepayAmount = GetArg("-anonymizebytepayamount", 0);
    if(nAnonymizeBytepayAmount > 999999) nAnonymizeBytepayAmount = 999999;
    if(nAnonymizeBytepayAmount < 2) nAnonymizeBytepayAmount = 2;
    ...
    LogPrintf("Anonymize Bytepay Amount %d\n", nAnonymizeBytepayAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BEPAY+1000 == (.1BEPAY+100)*10
       10BEPAY+10000 == (1BEPAY+1000)*10
    */
