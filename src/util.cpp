...
int nAnonymizeBytepayAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bytepay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bytepay
    // Mac: ~/Library/Application Support/Bytepay
    // Unix: ~/.bytepay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bytepay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bytepay.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bytepayd.pid"));
