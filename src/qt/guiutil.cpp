...
{
    // Convert bytepay:// to bytepay:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bytepay://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "bytepay:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bytepay.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bytepay.lnk
...
{
    return GetAutostartDir() / "bytepay.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a bytepay.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bytepay\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bytepay app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add bytepay app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bytepay-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bytepay-qt [" + tr("command-line options") + "]                     " + "\n";
