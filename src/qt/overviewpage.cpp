...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBytepayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBytepayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBytepayAmount = strAnonymizeBytepayAmount.remove(strAnonymizeBytepayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBytepayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBytepayAmount*COIN) nMaxToAnonymize = nAnonymizeBytepayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBytepayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBytepayAmount));
        strAnonymizeBytepayAmount = strAnonymizeBytepayAmount.remove(strAnonymizeBytepayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBytepayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
