...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBytepayAmount"))
        settings.setValue("nAnonymizeBytepayAmount", 1000);
    nAnonymizeBytepayAmount = settings.value("nAnonymizeBytepayAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBytepayAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBytepayAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBytepayAmount:
            return QVariant(nAnonymizeBytepayAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBytepayAmount:
            nAnonymizeBytepayAmount = value.toInt();
            settings.setValue("nAnonymizeBytepayAmount", nAnonymizeBytepayAmount);
            emit AnonymizeBytepayAmountChanged(nAnonymizeBytepayAmount);
            break;
        default:
            break;
        }
